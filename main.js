...
function setDifficulty(value) {
    switch (value) {
        case 1:
            ...
            animationSpeed = 120; // Зміна швидкості анімації для легкого рівня
            targetFood = 15;
            break;
        case 2:
            difficultyDisplay.textContent = "Нормальний";
            animationSpeed = 60; // Зміна швидкості анімації для нормального рівня
            targetFood = 20;
            break;
        case 3:
            difficultyDisplay.textContent = "Складний";
            animationSpeed = 30; // Зміна швидкості анімації для складного рівня
            targetFood = 25;
            break;
        default:
            break;
    ...
    // Оновлення швидкості анімації для всіх елементів з класом food-images
    const foodImages = document.querySelector('#food-images');
    foodImages.style.animationDuration = `${animationSpeed}s`;
...
// Функція для відображення рейтингів гравців
function showPlayerRecords() {
    let playerRecords = JSON.parse(localStorage.getItem('playerRecords')) || [];
    playerRecords.sort((a, b) => a.score - b.score); // Сортуємо за результатами
    // let rank = 1;
    let recordsHTML = '<h2>Рейтинг гравців:</h2><ol>';
    playerRecords.forEach((record) => {
        recordsHTML += `<li>${record.name}: ${record.score} секунд</li>`;
        // rank++;
    });
    recordsHTML += '</ol>';
    gameArea.innerHTML = recordsHTML;
}
...
function finishGame(playerName) {
    ...
    savePlayerRecord(playerName, seconds);
}
